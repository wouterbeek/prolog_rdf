...
q__p(P, G) :-
  ...
  dcg_with_output_to(
    section((
      "Predicate ",
      sq(dcg_rdf_print_predicate(P)),
      " in graph ",
      sq(dcg_rdf_print_graph_term(G))
    ))
  ),
  q__p_os0(hdt, P, G),
  q__p_ds0(hdt, P, G).
...
q__ps(G) :-
  q_store_graph(data, G),
  dcg_with_output_to(section(("Graph ",dcg_rdf_print_graph_term(G)))),
  findall(
...
q__x(Base, S, P, O, G) :-
  ...
  cli_pagination_result(Result, rdf_print_quads).
