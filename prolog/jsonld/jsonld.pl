:- module(
  jsonld,
  [
    jsonld_abbreviate/2, % +Iri, -Abbreviation
...
@tbd Regenerate JSON-LD for VZM to add alias definition for RDF datatype IRIs.
@version 2016/01
*/
...
jsonld_subject(S, Json) :-
  ...
  jsonld_triples(Triples, Json).



%! jsonld_triples(+Json, -Triples) is det.

jsonld_triples(Json, Triples) :-
  nonvar(Json), !,
...
jsonld_oterm(PDefs, _, P2, O1, Elems2) :-
  ...
  rdf_list(O1, Elems1),
  maplist(jsonld_abbreviate, Elems1, Elems2).
