...
% @tbd What to do with double ZM instantiation?
...
wkt(C, L) -->
  {wkt_class_name(C, Name), Comp =.. [Name,L]},
  'well-known text representation'(_, Comp).

wkt_class_name(geold:'MultiPolygon', multipolygon).
wkt_class_name(geold:'Polygon', polygon).
