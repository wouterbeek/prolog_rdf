...
qu_change_iri_prefix0(Alias1, Alias2), atom(Prefix2), Cs -->
  {rdf_alias_prefix(Alias1, Prefix1)},
  atom(Prefix1),
  {rdf_alias_prefix(Alias2, Prefix2)},
  rest(Cs).
...
qu_replace_predicate_deb(P, Q) -->
  ...
  sq(dcg_rdf_print_predicate(P)),
  " → ",
  sq(dcg_rdf_print_predicate(Q)),
  ".".


qu_replace_predicates_by_vocab_deb(GVocab) -->
  "Replace predicates based on vocabulary ",
  sq(dcg_rdf_print_graph_term(GVocab)),
  ".".
