...
We introduce the following terminology:
  ...
  * **IRI prefix**
    ...
    The prefix of an IRI that is abbreviated by an alias.

  * **Local Name**

    For an IRI, the suffix that is not covered by the IRI-prefix
    denoted by an alias.

Notice that we deviate from the terminology in the RDF 1.1
specification, where aliases are -- quite misleadingly! -- called
*prefixes*.

We can distinguish between the follow two kinds of IRI terms:

  * **Unaliases IRI term**

    An IRI term where no IRI prefix is substituted by an alias.

  * **Aliased IRI term**

    An IRI term where an IRI prefix is subsituted by an alias.
    Aliased IRI terms are of the form ‘Alias:LocalName’.

In practice aliases are often used to *shorten* IRI terms.
...
:- initialization(init_rdf_alias).

init_rdf_alias :-
  
  % PREDEFINED FOR RDFa
  %
  % The following aliases are already defined by module
  % ‘semweb/rdf_db’: ‘owl’, ‘rdf’, ‘rdfs’, ‘skos’, ‘void’, ‘xsd’.
  ...
  rdf_create_alias(rdft, 'http://www.w3.org/ns/rdftest#'),
  ...
  % DEFINED BY W3C
  rdf_create_alias(describedby, 'http://www.w3.org/2007/05/powder-s#describedby'),
  rdf_create_alias(fn, 'http://www.w3.org/2005/xpath-functions# '),
  rdf_create_alias(license, 'http://www.w3.org/1999/xhtml/vocab#license'),
  rdf_create_alias(role, 'http://www.w3.org/1999/xhtml/vocab#role'),
  rdf_create_alias(sfn, ' http://www.w3.org/ns/sparql#'),

  % WIDELY USED
  %
  % The following aliases are already defined by module
  % ‘semweb/rdf_db’: ‘dc’, ‘dcterms’, ‘eor’, ‘foaf’, ‘serql’.
  rdf_create_alias(cc, 'http://creativecommons.org/ns#'),
  rdf_create_alias(ctag, 'http://commontag.org/ns#'),
  %dc http://purl.org/dc/elements/1.1/
  rdf_create_alias(dct, 'http://purl.org/dc/terms/'),
  %dcterms http://purl.org/dc/terms/
  %eor http://dublincore.org/2000/03/13/eor#
  %foaf http://xmlns.com/foaf/0.1/
  rdf_create_alias(gr, 'http://purl.org/goodrelations/v1#'),
  rdf_create_alias(ical, 'http://www.w3.org/2002/12/cal/icaltzd#'),
  rdf_create_alias(og, 'http://ogp.me/ns#'),
  ...
  rdf_create_alias(v, 'http://rdf.data-vocabulary.org/#'),
  rdf_create_alias(vcard, 'http://www.w3.org/2006/vcard/ns#'),
  rdf_create_alias(schema, 'http://schema.org/'),

  % UNDER DEVELOPMENT
  rdf_create_alias(dqv, 'http://www.w3.org/ns/dqv#'),
  rdf_create_alias(earl, 'http://www.w3.org/ns/earl#'),
  rdf_create_alias(ssn, 'http://www.w3.org/ns/ssn/'),
  rdf_create_alias(oa, 'http://www.w3.org/ns/oa#'),

  % LOD LAUNDROMAT
  rdf_create_alias(llm, 'http://lodlaundromat.org/metrics/ontology/'),
  rdf_create_alias(llo, 'http://lodlaundromat.org/ontology/'),
  rdf_create_alias(llr, 'http://lodlaundromat.org/resource/'),

  % QUINE
  rdf_create_alias(qp, 'https://quine.cc/property/'),
  rdf_create_alias(qr, 'https://quine.cc/resource/'),

  % TRIPLY
  rdf_create_alias(triply, 'https://api.dev.triply.cc/'),
  
  % CUSTOM
  rdf_create_alias(bibframe, 'http://bibframe.org/vocab/'),
  rdf_create_alias(blog, 'http://quine.cc/blog/def#'),
  rdf_create_alias(cache, 'http://quine.cc/cache/'),
  ...
  rdf_create_alias(dby, 'http://dbpedia.org/class/yago/'),
  rdf_create_alias(ex, 'http://example.org/'),
  rdf_create_alias(fb, 'http://ogp.me/ns/fb#'),
  rdf_create_alias(formats, 'http://www.w3.org/ns/formats/'),
  rdf_create_alias(geold, 'http://geojsonld.com/vocab#'), % @tbd
  rdf_create_alias(geosparql, 'http://www.opengis.net/ont/geosparql#'),
  rdf_create_alias(http, 'http://www.w3.org/2011/http#'),
  rdf_create_alias(odp, 'http://www.ontologydesignpatterns.org/'),
  rdf_create_alias(sparql, 'http://www.w3.org/2005/sparql-results#'),
  rdf_create_alias('umbel-rc', 'http://umbel.org/umbel/rc/'),
  rdf_create_alias('wiki-entity', 'http://www.wikidata.org/entity/'),
  rdf_create_alias(wgs84, 'http://www.w3.org/2003/01/geo/wgs84_pos#'),
  rdf_create_alias('wiki-p', 'http://www.wikidata.org/prop/'),
  rdf_create_alias(wkt, 'http://geojsonld.com/wkt#'),
  rdf_create_alias(wv, 'http://vocab.org/waiver/terms/norms'),
  rdf_create_alias(yago, 'http://yago-knowledge.org/resource/'),
...
dbpedia_register(LTag) :-
  ...
  uri_comps(Prefix1, uri(http,Auth,[resource,'Category:'],_,_)),
  rdf_reset_prefix(Alias1, Prefix1),

  atomic_list_concat([LTag,dbp], ., Alias2),
  uri_comps(Prefix2, uri(http,Auth,['property/'],_,_)),
  rdf_reset_prefix(Alias2, Prefix2),
  
  atomic_list_concat([LTag,dbr], ., Alias3),
  uri_comps(Prefix3, uri(http,Auth,['resource/'],_,_)),
  rdf_reset_prefix(Alias3, Prefix3).



%! rdf_reset_prefix(+Alias, +Prefix) is det.
%
...
rdf_reset_prefix(Alias, Prefix) :-
  with_mutex(rdf_reset_prefix, (
    (   rdf_alias_prefix(Alias, OldPrefix)
    ->  (   OldPrefix == Prefix
        ->  true
        ;   rdf_create_alias(Alias, Prefix),
            msg_warning(
              "Alias ~a was reset from prefix ~a to prefix ~a.",
              [Alias,OldPrefix,Prefix]
            )
        )
    ;   rdf_create_alias(Alias, Prefix)
    )
