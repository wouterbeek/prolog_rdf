...
@author Wouter Beek
@version 2015/07, 2015/10, 2015/12-2016/03, 2016/12-2017/01
*/
...
%! rdf_edges_to_export_graph(+Es, -ExportG, +Opts) is det.
%
...
%! rdf_graph_to_export_graph(+G, -ExportG) is det.
...
%! rdf_term_to_export_graph(+M, +Term, -ExportG, +Opts) is det.

rdf_term_to_export_graph(M, Term, ExportG, Opts) :-
  rdf_cbd_triples(M, Term, Triples),
...
%! rdf_edge_arrowhead(+E, -ArrowHead) is det.
...
rdf_edge_label(edge(_,P,_)) --> dcg_rdf_print_term(P).



%! rdf_edge_style(+E, -Style) is det.
...
rdf_vertex_image(M, G, V, Img) :-
  once(rdf_image(M, V, Img, G)).



%! rdf_vertex_label(+Opts, +Term, -Lbl) is det.

rdf_vertex_label(_Opts1, V) -->
  %{merge_options(Opts1, [literal_ellipsis(50)], Opts2)},
  dcg_rdf_print_term(V).



%! rdf_vertex_peripheries(+Term, -Peripheries:nonnneg) is det.
...
%! rdf_vertex_shape(+Term, -Shape) is det.
