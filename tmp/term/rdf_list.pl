...
rdf_list(RdfList, [H|T], G, Opts):-
  % `rdf:first`
  rdf(RdfList, rdf:first, First, G:_),
  (   % Nested list.
      option(recursive(true), Opts, true),
      rdf_list(First)
  ->  rdf_list(First, H, G, Opts)
  ;   % Dedicated support for RDF datatypes.
      option(datatype(D), Opts)
  ->  rdf_literal(First, rdf:value, D, H, G)
  ;   H = First
  ),
